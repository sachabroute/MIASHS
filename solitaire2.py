...
def random_jeu_sol(cartes_alea) :
    ...
    while j < int(len(cartes_alea)/2) :
        ...
        for k in range(j,j+i-1) :
            print("Numéro de colonne :", i, "Numéro de carte :", k+1, "Numéro de carte totale :", j+1)
            ...
            for i in range(len(game)-2) :
                for j in range(len(game[i])) :
                    if game[i][j][1] == 0 :
                        ...
                        fenetre.blit(dico_images[cartes_alea[i]], (i*85+400,j*25+213))
                        last_column = i*85+400
